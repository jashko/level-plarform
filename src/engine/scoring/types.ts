...
export interface CityHousingMarketInputs {
  ...
  /**
   * Соотношение распроданности к стройготовности, % (ось X карты ДОМ.РФ).
   * <60% = дефицит спроса, 60-80% = баланс, >80% = дефицит предложения.
   * Источник: ЕИСЖС / расчёты ДОМ.РФ на 01.01.2026.
   */
  sellReadinessRatioPct?: number;
  /**
   * Срок реализации непроданного строящегося жилья, лет (ось Y карты ДОМ.РФ).
   * Чем меньше — тем активнее спрос. <3 лет = хорошо, >5 лет = дефицит спроса.
   * Источник: ЕИСЖС / расчёты ДОМ.РФ на 01.01.2026.
   */
  unsoldYearsOfSupply?: number;
}
